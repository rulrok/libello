...
class processosDAO extends abstractDAO {
    ...
    public function pesquisarImagem($termoBusca, $limite = "", $acessoTotal = false) {
        ...
        $sql = "SELECT"
                ...
                . '( SELECT idDescritor FROM processo_arvore as b WHERE b.nome RLIKE :termoBusca AND (b.idDescritor = a.descritor1 OR b.idDescritor = a.descritor2 OR b.idDescritor = a.descritor3 OR b.idDescritor = a.descritor4)))'
                . ') ORDER BY idImagem ) as t'
                . ' JOIN `processo_arvore` t1 ON t1.idDescritor = t.descritor1'
                . ' JOIN `processo_arvore` t2 ON t2.idDescritor = t.descritor2'
                . ' JOIN `processo_arvore` t3 ON t3.idDescritor = t.descritor3'
                . ' JOIN `processo_arvore` t4 ON t4.idDescritor = t.descritor4 '
                . "$query_auxiliar"
                . $limite;
    ...
    public function consultarDescritoresNivel1($colunas = "*", $condicao = null, $condicaoJuncao = null) {
        ...
        $sql = "SELECT $colunas FROM processo_arvore $condicaoJuncao $condicao";
        return $this->executarSelect($sql);
    }
    ...
    public function consultarDescritoresCompletos($idDescritorExcluir = null) {
        try {
            ...
            if ($idDescritorExcluir !== null) {
                ...
                $sql = 'INSERT INTO ids_aux(id) SELECT idDescritor FROM processo_arvore WHERE nivel = 1 AND idDescritor <> :idDescritorExcluir';
                $this->executarQuery($sql, $param);
                //Seleciona os filhos de nível 2 que os pais foram selecionados
                $sql = 'INSERT INTO ids_aux2(id) SELECT idDescritor FROM processo_arvore WHERE pai IN (SELECT id FROM ids_aux) AND idDescritor <> :idDescritorExcluir';
                $this->executarQuery($sql, $param);
                ;
                ...
                $sql = 'INSERT INTO ids_aux(id) SELECT idDescritor FROM processo_arvore WHERE pai IN (SELECT id FROM ids_aux2) AND idDescritor <> :idDescritorExcluir';
                $this->executarQuery($sql, $param);
                ...
                $sql = 'INSERT INTO ids(id,pai) SELECT idDescritor,pai FROM processo_arvore WHERE idDescritor IN (SELECT id FROM ids_aux2)';
                $this->executarQuery($sql);
                //Seleciona os pais de nível 3
                $sql = 'DELETE QUICK FROM ids_aux';
                $this->executarQuery($sql);
                $sql = 'INSERT INTO ids_aux(id,pai) SELECT idDescritor,pai FROM processo_arvore WHERE idDescritor IN (SELECT pai FROM ids)';
                $this->executarQuery($sql);
                ...
                $sql = 'DELETE QUICK FROM ids_aux2';
                $this->executarQuery($sql);
                $sql = 'INSERT INTO ids_aux2(id,pai) SELECT idDescritor,pai FROM processo_arvore WHERE idDescritor IN (SELECT pai FROM ids_aux)';
                $this->executarQuery($sql);
                ...
                $sql = 'DELETE QUICK FROM ids_aux';
                $this->executarQuery($sql);
                $sql = 'INSERT INTO ids_aux(id,pai) SELECT idDescritor,pai FROM processo_arvore WHERE idDescritor IN (SELECT pai FROM ids_aux2)';
                $this->executarQuery($sql);
                $sql = 'INSERT INTO ids(id,pai) SELECT * FROM ids_aux';
                $this->executarQuery($sql);
            } else {
                //Seleciona os filhos propriamente ditos
                $sql = "INSERT INTO ids(id) SELECT DISTINCT idDescritor FROM processo_arvore WHERE nivel = 4";
                $this->executarQuery($sql);
                //Para encontrar os pais dos descritores nivel 4
                $sql = "INSERT INTO ids_aux(id) SELECT DISTINCT pai FROM processo_arvore WHERE nivel = 4";
                $this->executarQuery($sql);
                //Seleciona os pais nivel 3
                $sql = 'INSERT INTO ids(id) SELECT DISTINCT idDescritor FROM processo_arvore WHERE idDescritor IN (SELECT id FROM ids_aux)';
                $this->executarQuery($sql);
                //Para encontrar os pais dos descritores nivel 3
                $sql = 'INSERT INTO ids_aux2(id) SELECT DISTINCT pai FROM processo_arvore WHERE idDescritor IN (SELECT id FROM ids_aux)';
                $this->executarQuery($sql);
                //Seleciona os pais nivel 2
                $sql = 'INSERT INTO ids(id) SELECT DISTINCT idDescritor FROM processo_arvore WHERE idDescritor IN (SELECT id FROM ids_aux2)';
                $this->executarQuery($sql);
                $sql = 'DELETE QUICK FROM ids_aux;';
                $this->executarQuery($sql);
                //Para encontrar os pais dos descritores nivel 2
                $sql = 'INSERT INTO ids_aux(id) SELECT DISTINCT pai FROM processo_arvore WHERE idDescritor IN (SELECT id FROM ids_aux2)';
                $this->executarQuery($sql);
                //Seleciona os pais nivel 1
                $sql = 'INSERT INTO ids(id) SELECT DISTINCT idDescritor FROM processo_arvore WHERE idDescritor IN (SELECT id FROM ids_aux)';
                $this->executarQuery($sql);
            }
            $sql = 'SELECT * FROM processo_arvore WHERE idDescritor IN (SELECT id FROM ids)';
    ...
    public function atualizarDescritor($idDescritor, Descritor $novosDados) {
        ...
        $sql = "UPDATE processo_arvore SET nome = :nome WHERE idDescritor = :idDescritor";
        $params = array(
    ...
    public function recuperarDescritor($idDescritor) {
        ...
        $sql = "SELECT * from processo_arvore WHERE idDescritor = :idDescritor";
        $params = array(
            ':idDescritor' => [$idDescritor, PDO::PARAM_INT]
        );
...
//                $sqlAux = "SELECT pai FROM processo_arvore WHERE idDescritor = :idDescritor LIMIT 1";
//                $paramsAux = array(
...
//            array_pop($sequanciaAntigaIds);
            ...
            $sqlRotulo = "SELECT IFNULL ( (SELECT rotulo FROM processo_arvore WHERE pai = :novoPai ORDER BY rotulo DESC LIMIT 1) ,0)";
            $paramsRotulo = array(
            ...
            //  Mover descritor
            //------------------------------------------------------------------
            $sql = "UPDATE processo_arvore SET pai = :novoPai,rotulo = :novoRotulo WHERE idDescritor = :idDescritor AND pai = :antigoPai";
            $params = array(
                ':novoPai' => [$novoPai, PDO::PARAM_INT]
            ...
            $sql2 = "UPDATE processo_arvore SET qtdFilhos = qtdFilhos - 1 WHERE idDescritor = :antigoPai";
            $params2 = array(
                ':antigoPai' => [$antigoPai, PDO::PARAM_INT]
            );
            ...
            $sql3 = "UPDATE processo_arvore SET qtdFilhos = qtdFilhos + 1 WHERE idDescritor = :novoPai";
            $params3 = array(
                ':novoPai' => [$novoPai, PDO::PARAM_INT]
            );
    ...
    public function arvoreProcessos($completaApenas = false, $idDescritorExcluir = null) {
        $arvore[] = ['id' => fnEncrypt(ImagensDescritor::ID_RAIZ_NIVEL_ZERO), 'parent' => '#', 'text' => 'Processos', 'nivel' => '0', 'rotulo' => '0', 'state' => ['opened' => true, 'selected' => true]];
        if (!$completaApenas) {
