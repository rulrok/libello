...
class viagemDAO extends abstractDAO {
    ...
    public static function inserir(Viagem $valueObject) {
        ...
        $sql = "INSERT INTO viagem(idCurso,idPolo,responsavel, dataIda, horaIda, dataVolta, horaVolta, motivo, estado, diarias, outroDestino)";
//        $sql .= " VALUES (" . $idCurso . ",'" . $idPolo . $s . $responsavel . $s . $dataIda . $s . $horaIda . $s . $dataVolta . $s . $horaVolta . $s . $motivo . $s . $estado . $s . $diarias . "')";
        $sql .= " VALUES (:idCurso, :idPolo, :responsavel, :dataIda, :horaIda, :dataVolta, :horaVolta, :motivo, :estado, :diarias, :outroDestino);";
        try {
